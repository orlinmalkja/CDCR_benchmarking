...
def train_model(document_clustering, train_set, dev_set, output_dir, config):
    '''
    ...
    for epoch in range(0, config_dict["epochs"]):
        ...
        for topic_id in topics_keys:
            ...
            # extract the entity and event mentions of a topic
            ...
            entity_clusters = []  # will contain the clusters of mentions across all docs for a particular topic
            for doc_id, clusters in wd_entity_clusters.items():
                entity_clusters.extend(clusters)

            event_clusters = init_cd(event_mentions, is_event=True)  # a list of cross document singleton clusters - a cluster which contains one single mention

            # initialize cluster representation
            entity_clusters = update_lexical_vectors(entity_clusters, cd_entity_model, device,
                                   is_event=False, requires_grad=False)
            event_clusters = update_lexical_vectors(event_clusters, cd_event_model, device,
            ...
            for i in range(1,config_dict["merge_iters"]+1):
                ...
                cd_entity_model = train_and_merge(clusters=entity_clusters, other_clusters=event_clusters,
                ...
                cd_event_model = train_and_merge(clusters=event_clusters, other_clusters=entity_clusters,
        ...
        for event_threshold in threshold_list:
            for entity_threshold in threshold_list:
                ...
                _ ,event_f1, _, _ = test_models(document_clustering, dev_set, cd_event_model, best_saved_cd_entity_model, device,
                ...
                entity_f1, _ , _, _= test_models(document_clustering, dev_set, best_saved_cd_event_model, cd_entity_model, device,
                                                  config_dict, write_clusters=False, out_dir=out_dir,
                                                  doc_to_entity_mentions=doc_to_entity_mentions, analyze_scores=False)
        ...
        save_epoch_f1(event_f1, entity_f1, epoch, best_event_th, best_entity_th)  #Orlin: commented this code: Reason UnboundLocalError: local variable 'best_event_th' referenced before assignment, which I think its a problem of the data chosen 
...
def train_and_merge(clusters, other_clusters, model, optimizer,
    ...
    clusters = update_args_feature_vectors(clusters, other_clusters, model, device, is_event)
    ...
    model = train(cluster_pairs, model, optimizer, loss,
          device, topic.docs, epoch, topics_counter, topics_num, config_dict, is_event,
           other_clusters)

    with torch.no_grad():
        clusters = update_lexical_vectors(clusters, model, device ,is_event, requires_grad=False)

        event_mentions, entity_mentions = topic_to_mention_list(topic, is_gold=True)

        # Update span representations after training
        event_mentions = create_mention_span_representations(event_mentions, model, device, topic.docs, is_event=True,
                                            requires_grad=False)
        entity_mentions = create_mention_span_representations(entity_mentions, model, device, topic.docs, is_event=False,
                                            requires_grad=False)
