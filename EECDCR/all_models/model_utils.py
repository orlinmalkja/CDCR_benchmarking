...
def create_predicates_features_vec(mention_1, mention_2, event_clusters, device, model):
    ...
    :param event_clusters: current event clusters
    :param device: Pytorch device (cpu/gpu)
    :param model: CDCorefScorer object
    :return: a vector for four binary features embedded as a tensor of size (1,200),
    each feature embedded as 50 dimensional embedding.
...
def find_mention_cluster_vec(mention_id, clusters):
    ...
    for cluster in clusters:
        if mention_id in cluster.mentions:
            return cluster.lex_vec.detach(), cluster.mentions
    
    return False, []  # the mention_id is not part of cluster
...
def create_event_cluster_bow_arg_vec(event_cluster, entity_clusters, model, device):
    ...
    for event_mention in event_cluster.mentions.values():
        ...
        if event_mention.arg0 is not None:
            arg_vec, _ = find_mention_cluster_vec(event_mention.arg0[1], entity_clusters)
            event_mention.arg0_vec = arg_vec.to(device)
        if event_mention.arg1 is not None:
            arg_vec, _ = find_mention_cluster_vec(event_mention.arg1[1], entity_clusters)
            event_mention.arg1_vec = arg_vec.to(device)
        if event_mention.amtmp is not None:
            arg_vec, _ = find_mention_cluster_vec(event_mention.amtmp[1], entity_clusters)
            event_mention.time_vec = arg_vec.to(device)
        if event_mention.amloc is not None:
            arg_vec, _ = find_mention_cluster_vec(event_mention.amloc[1], entity_clusters)
...
def create_entity_cluster_bow_predicate_vec(entity_cluster, event_clusters, model, device):
    ...
    for entity_mention in entity_cluster.mentions.values():
        ...
        for predicate_id, rel in predicates_dict.items():
            ...
            pred_vec, cluster_mentions = find_mention_cluster_vec(predicate_id[1], event_clusters)

            if pred_vec != None and type(pred_vec) != bool:
                if rel == 'A0':
                ...
                if pred_vec == None:
                    LOGGER.warning("The semantic vector of clusters mentions{} related to the mention {} is None", cluster_mentions, predicate_id[1])  
                elif type(pred_vec) == bool:
                    LOGGER.warning("the mention_id", predicate_id[1], "is not part of any cluster") 


    LOGGER.info("Ending the function : create_entity_cluster_bow_predicate_vec()")   
    ...
    for cluster in clusters:
        if is_event:
            ...
            lex_vec = create_entity_cluster_bow_lexical_vec(cluster, model, device,
                                                            ...
                                                            requires_grad=requires_grad)
        if lex_vec == None:
            LOGGER.warning("The lex vex vec for this cluster is None", cluster)
            LOGGER.warning("The mentions of this cluster are", cluster.mentions)
...
def update_args_feature_vectors(clusters, other_clusters, model, device, is_event):
    ...
    '''
    updated_clusters = [] # the clusters with updated args_feature_vectors
    for cluster in clusters:
        if is_event:
            # Use an average of span representations to represent arguments/predicates clusters.
            event_cluster = create_event_cluster_bow_arg_vec(cluster, other_clusters, model, device)
            updated_clusters.append(event_cluster)
        else:
            entity_cluster = create_entity_cluster_bow_predicate_vec(cluster, other_clusters, model, device)
            updated_clusters.append(entity_cluster)
...
def mention_pair_to_model_input(pair, model, device, topic_docs, is_event, requires_grad,
    ...
    if use_binary_feats:
        if is_event:   # clusters will be of same type as the Entity or Event model that we are training
...
def test_pairs_batch_to_model_input(batch_pairs, model, device, topic_docs, is_event,
                                    use_args_feats, use_binary_feats, clusters, other_clusters):
    '''
...
def merge_clusters(pair_to_merge, clusters, other_clusters, is_event,
    ...
    new_cluster = update_args_feature_vectors([new_cluster], other_clusters, model, device, is_event)[0]  
...
def assign_score(cluster_pair, model, device, topic_docs, is_event, use_args_feats,
    ...
    for batch_pairs in batches:
        batch_tensor = test_pairs_batch_to_model_input(batch_pairs, model, device,
                                                       ...
                                                       use_args_feats=use_args_feats,
                                                       use_binary_feats=use_binary_feats,
                                                       clusters = clusters, other_clusters=other_clusters)
    ...
    while True:
        ...
        if max_score > threshold:
            ...
            logging.info('epoch {} topic {}/{} - merge {} clusters with score {} clusters : {} {}'.format(
                epoch, topics_counter, topics_num, mode, str(max_score), str(max_pair[0]),
                str(max_pair[1])))
            clusters, pairs_dict = merge_clusters(max_pair, clusters, other_clusters, is_event,
...
def test_models(document_clustering, test_set, cd_event_model, cd_entity_model, device,
    ...
    :param document_clustering: boolean- enable/disable document clustering
    ...
    with torch.no_grad():
        for topic_id in topics_keys:
            ...
            event_mentions = create_mention_span_representations(event_mentions, cd_event_model, device,
                                                topic.docs, is_event=True,
                                                requires_grad=False)
            entity_mentions = create_mention_span_representations(entity_mentions, cd_entity_model, device,
            ...
            if write_clusters:
                ...
                with open(os.path.join(out_dir, 'entity_clusters.txt'), 'a', encoding="utf8") as entity_file_obj:
                ...
                with open(os.path.join(out_dir, 'event_clusters.txt'), 'a', encoding="utf8") as event_file_obj:
    ...
    if config_dict["test_use_gold_mentions"]:
        ...
        #return event_b3_f1, entity_b3_f1, all_entity_mentions, all_event_mentions
    else:
    ...
    return entity_b3_f1, event_b3_f1, all_entity_clusters, all_event_clusters
...
def mention_data_to_string(mention, other_clusters, is_event, topic_docs):
    ...
    doc_id = mention.doc_id.split("/")[2]
    strings.append('sent: {}'.format(topic_docs[doc_id].sentences[mention.sent_id].get_raw_sentence()))
    return '\n'.join(strings)
