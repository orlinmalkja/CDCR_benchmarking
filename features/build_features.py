...
#
# parser.add_argument('--config_path', type=str, default='newsalyze/entities/eecdcr/orig_config/newsalyze_config.json',
...
def have_string_match(mention, arg_str, arg_start, arg_end):
    ...
    if arg_start >= mention.start_offset and arg_end <= mention.end_offset:  # the mention span contains the mention span
        return True
    if len(set(mention.tokens_numbers).intersection(set(
            range(arg_start, arg_end + 1)))) > 0:  # intersection between the mention's tokens and the argument's tokens
        return True
    return False
...
#                             if find_argument(rel_name, rel_tokens, matched_event, sent_entities, sent, is_gold,
#                                              srl_obj):
#                                 matched_args_count += 1
#                     else:
...
def set_elmo_embed_to_mention(mention, sent_embeddings):
    ...
    if head_index !=None and head_index < len(sent_embeddings):
        head_embeddings = torch.from_numpy(sent_embeddings[int(head_index)])
        
        if head_embeddings != None:
            mention.head_elmo_embeddings = head_embeddings
...
def set_elmo_embeddings_to_mentions(elmo_embedder, sentence, set_pred_mentions):
    ...
    gold_event_mentions = sentence.gold_event_mentions
    gold_entity_mentions = sentence.gold_entity_mentions

    new_gold_event_mentions = []
    new_gold_entity_mentions = []

    for i in range(len(gold_event_mentions)):
        event = gold_event_mentions[i]
        resp = set_elmo_embed_to_mention(event, avg_sent_embeddings)
        if resp == False:
            LOGGER.warning("mention with ID",event.mention_id,"with a content of ",event.mention_str,"is either None or has an index greater the number of tokens or its embedding is empty")

        else:
            event = resp  # we actually return the event after having assigned its head embedding
            new_gold_event_mentions.append(event)

    for i in range(len(gold_entity_mentions)):
        entity = gold_entity_mentions[i]
        resp = set_elmo_embed_to_mention(entity, avg_sent_embeddings)
        if resp == False:
              LOGGER.warning("mention with ID",entity.mention_id,"with a content of ",entity.mention_str,"is either None or has an index greater the number of tokens or its embedding is empty")

        else:
            entity = resp  # we actually return the event after having assigned its head embedding
            new_gold_entity_mentions.append(entity)

    sentence.gold_event_mentions = new_gold_event_mentions
    sentence.gold_entity_mentions = new_gold_entity_mentions


    # Set the contextualized vector also for predicted mentions
    if set_pred_mentions:
        pred_event_mentions = sentence.pred_event_mentions
        pred_entity_mentions = sentence.pred_entity_mentions

        new_pred_event_mentions = []
        new_pred_entity_mentions = []

        for i in range(len(pred_event_mentions)):
            event = pred_event_mentions[i]
            resp = set_elmo_embed_to_mention(event, avg_sent_embeddings)  # set the head contextualized vector
            if resp == False:
                sentence.pred_event_mentions.remove(event)
                LOGGER.warning("mention with ID",event.mention_id,"with a content of ",event.mention_str,"is either None or has an index greater the number of tokens or its embedding is empty")
                LOGGER.info(
                    "mention with ID",event.mention_id,"with a content of ",event.mention_str,"is either None or has an index greater the number of tokens or its embedding is empty")

            else:
                event = resp  # we actually return the event after having assigned its head embedding
                new_pred_event_mentions.append(event)   
            

        for i in range (len(pred_entity_mentions)):
            entity = pred_entity_mentions[i]
            resp = set_elmo_embed_to_mention(entity, avg_sent_embeddings)  # set the head contextualized vector
            if resp == False:
                LOGGER.warning("mention with ID",entity.mention_id,"with a content of ",entity.mention_str,"is either None or has an index greater the number of tokens or its embedding is empty")

            else:
                entity = resp  # we actually return the event after having assigned its head embedding
                new_pred_entity_mentions.append(entity)

        sentence.pred_event_mentions = new_pred_event_mentions
        sentence.pred_entity_mentions = new_pred_entity_mentions
...
def load_elmo_embeddings(dataset, elmo_embedder, set_pred_mentions):
    ...
    for topic_id, topic in tqdm(dataset.topics.items(), desc='Loading Elmo embeddings'):
        for doc_id, doc in topic.docs.items():
            for sent_id, sent in doc.get_sentences().items():
                doc.sentences[sent_id] = set_elmo_embeddings_to_mentions(elmo_embedder, sent, set_pred_mentions)
            topic.docs[doc_id] = doc
        dataset.topics[topic_id] = topic
